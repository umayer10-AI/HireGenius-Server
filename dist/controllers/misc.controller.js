...
export const listApplications = asyncHandler(async (req, res) => {
    const result = await applicationService.list(req.query, req.user);
...
export const listNotifications = asyncHandler(async (req, res) => {
    ...
    return sendSuccess(res, { items: result.data, unreadCount: result.unreadCount }, "Notifications fetched", 200, buildPaginationMeta(page, limit, result.total));
});
...
export const getDashboard = asyncHandler(async (req, res) => {
    ...
    if (role === "recruiter") {
        return sendSuccess(res, await dashboardService.getRecruiterDashboard(req.user), "Recruiter dashboard");
    }
    return sendSuccess(res, await dashboardService.getCandidateDashboard(req.user), "Candidate dashboard");
});
...
export const candidateMatch = asyncHandler(async (req, res) => {
...
export const listAIChats = asyncHandler(async (req, res) => {
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 20;
