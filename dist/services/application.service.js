...
import { sendApplicationStatusEmail, sendApplicationSubmittedEmail, } from "./email.service.js";
...
export class ApplicationService {
    async apply(input, candidate, resumeFile) {
        ...
        if (!resumeUrl)
            throw new ConflictError("Resume is required to apply");
        const now = new Date();
        ...
        await sendApplicationSubmittedEmail(candidate.name, candidate.email, job.title, company?.companyName || "Company");
        return application;
    }
    ...
    async updateStatus(id, updates, requester) {
        ...
        if (updates.status) {
            ...
            if (candidate) {
                ...
                await sendApplicationStatusEmail(candidate.name, candidate.email, job.title, updates.status);
            }
        }
        return updated;
    }
