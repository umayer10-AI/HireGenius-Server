...
export class UserService {
  ...
  async update(id: string, updates: Partial<UserDocument>, requester: UserDocument) {
  ...
  async setMyRole(user: UserDocument, role: "candidate" | "recruiter") {
  ...
  async deleteResume(userId: string) {
    const user = await userRepository.findByIdOrThrow(userId, "User not found");
    if (!user.resume) throw new NotFoundError("No resume found");
    await deleteCloudinaryAsset(user.resume);
    const updated = await userRepository.updateById(userId, {
  ...
  async getRecentlyViewedJobs(userId: ObjectId) {
    const user = await userRepository.findByIdOrThrow(userId, "User not found");
